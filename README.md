...
- Phase: Phase 12 promoted — formal full-flow runtime-test harness on top of Phase 10/11 readiness. The runtime now generates and verifies a run-owned full-stage controller flow with candidates, dispatch/validation ledgers, material/frontier state, local backflow closure, delivery-gate evidence, source-read-only snapshots, and exact-code negative probes. It remains a runtime-test capability, not a manuscript or submission-readiness claim.
