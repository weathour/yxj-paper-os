...
- Phase: Phase 10 promoted — real-subagent-run readiness with strict packet coverage for every dispatchable worker stage, run-owned packet binding, content validator registry, runtime-owned dry-run fixture, source-read-only filesystem snapshot proof excluding `.git/.omx`, and aggregate validation over inherited Phase 9/8/7/6 gates.
