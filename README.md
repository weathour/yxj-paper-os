...
- Phase: Phase 13 promoted — live native-subagent full-flow pilot on top of the Phase 12 formal harness. The runtime now records 40 real native-subagent lanes (producer + independent verifier for every canonical stage), ingests paired raw returns into stage-effect and live-validation records, verifies unique native thread provenance, source-read-only boundaries, authority modes, and exact-code negative probes. It remains a runtime-pilot capability, not a manuscript or submission-readiness claim.
