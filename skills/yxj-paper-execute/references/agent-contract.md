...
| `evidence-curator` | `evidence_and_method` | `direct_subagent` | `verifier` | claim-list, source-map, reader-spine-brief, NatureFigureContract when figure evidence is in scope | evidence-bank, claim-support-matrix, NaturePanelEvidenceMap | narrative | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_panel_evidence_map |
| `paper-architect` | `paper_architecture_and_narrative` | `direct_subagent` | `planner` | reader-spine-brief, object-representation-matrix, template-quant-profile, evidence-bank | contribution-map, section-blueprint, section-function-budget, NatureFigureContract, NatureFigureAestheticProfile | narrative, template | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_nature_figure_contract, validate_nature_figure_aesthetic_profile |
...
| `method-verifier` | `evidence_and_method` | `direct_subagent` | `verifier` | method-description, object-representation-matrix, evidence-bank, NatureFigureContract when figure panels are in scope | method-contract, experiment-validity-report, FigureSourceDataStatistics, FigureImageIntegrityRecord | narrative | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_figure_source_data_statistics, validate_figure_image_integrity_record |
| `figure-owner` | `manuscript_and_figure_production` | `direct_subagent` | `executor` | visual-table-algorithm-formula-budget, evidence-bank, object-representation-matrix, NatureFigureContract, NatureFigureAestheticProfile, NaturePanelEvidenceMap, FigureSourceDataStatistics, FigureImageIntegrityRecord, expression-design objects | figure-plan, caption-brief, visual-provenance-record, FigureBackendRoute, NatureCaptionLegendBrief | narrative, template, expression | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_expression_design_object_binding, validate_figure_backend_route, validate_nature_caption_legend, validate_rendered_pdf_surface_text |
| `review-director` | `review_and_governance` | `direct_subagent` | `critic` | manuscript-draft, reader-spine-brief, object-representation-matrix, template-quant-profile, Nature-grade figure package, rendered-surface gate | reader-experience-review-report, narrative-backflow-task, NatureFigureQAReport | narrative, template, expression when paper-facing | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_expression_design_object_binding, validate_nature_figure_qa_report, validate_rendered_pdf_surface_text |
...
| `export-owner` | `manuscript_and_figure_production` | `direct_subagent` | `executor` | validated-manuscript, figure-package, Nature-grade figure package, export-manifest, rendered-surface gate | export-package, readiness-report, FigureExportBundle | narrative, template, expression | validate_task_packet, validate_subagent_output, validate_task_status_transitions, validate_pua_telemetry, validate_agent_lane_department_binding, validate_task_material_io, validate_narrative_object_binding, validate_template_object_binding, validate_expression_design_object_binding, validate_figure_export_bundle, validate_rendered_pdf_surface_text |
...
- Nature-grade figure refs when final/export-facing figures are in scope:
  `NatureFigureContract`, `NatureFigureAestheticProfile`,
  `NaturePanelEvidenceMap`, `FigureBackendRoute`,
  `FigureSourceDataStatistics`, `FigureImageIntegrityRecord`,
  `NatureCaptionLegendBrief`, `NatureFigureQAReport`, `FigureExportBundle`, and
  `RenderedSurfaceGateReport` as applicable;
...
## Nature full-absorption lane bindings

M1 extends existing lanes with Nature-derived internal SOP materials. `skill-registry-owner` owns `NatureSourceInventory`, `CompanySkillRegistry`, and `NatureAbsorptionPackage`; `evidence-curator`/`sota-mapper`/`citation-banker`/`method-verifier` own reader, search, citation, and data availability materials; `paper-architect`/`exemplar-learner` own section moves and journal style; `manuscript-owner` owns polishing, presentation/PPT planning, and patent-draft boundaries; `review-director` owns reviewer panel and response action mapping. These are hidden internal material routes, not new public skills.
